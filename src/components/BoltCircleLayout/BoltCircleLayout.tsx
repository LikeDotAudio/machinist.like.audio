  ...
  return (
    <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(380px, 1fr))', gap: '12px', marginBottom: '12px', alignItems: 'start' }}>
        ...
        {/* 1. VISUAL: Interactive Pattern Preview (First in DOM order) */}
        <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '10px' }}>
            ...
            <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
              {holesCount} Holes @ {stepDeg.toFixed(1)}°
            </span>
          </div>
          ...
          <div style={{ background: 'var(--bg-tertiary)', padding: '12px 14px', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '1.1rem', fontWeight: 700, color: 'var(--accent-cyan)' }}>
                {chordalDist.toFixed(decPlaces)} <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{unitStr}</span>
              </span>
            </div>
        ...
        {/* 2. VARIABLES: Input Parameters (Second in DOM order) */}
        <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '10px' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>
              ⚙️ VARIABLES // Pattern Specification
            </h3>
          </div>
      ...
      {/* BOTTOM SECTION: 3. EXPLANATION & DRO TABLE (Third in DOM order) */}
      <div className="glass-panel">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '12px', marginBottom: '12px', flexWrap: 'wrap', gap: '10px' }}>
          <div>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--text-primary)', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span>📐 EXPLANATION // Calculated DRO Coordinates</span>
            </h3>
            <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
              Trigonometric Cartesian (X/Y) hole positions from origin ({centerX || 0}, {centerY || 0})
            </span>
          </div>
          <button
        ...
        {/* Coordinate Table */}
        <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-color)', maxHeight: '360px', overflowY: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.88rem' }}>
            <thead>
              <tr style={{ background: 'rgba(255, 255, 255, 0.04)', borderBottom: '1px solid var(--border-color)', position: 'sticky', top: 0, zIndex: 1, backdropFilter: 'blur(4px)' }}>
                <th style={{ padding: '8px 12px', color: 'var(--text-secondary)', fontWeight: 600 }}>Hole</th>
                <th style={{ padding: '8px 12px', color: 'var(--text-secondary)', fontWeight: 600 }}>Angle</th>
                <th style={{ padding: '8px 12px', color: 'var(--text-secondary)', fontWeight: 600, textAlign: 'right' }}>X Coord ({unitStr})</th>
                <th style={{ padding: '8px 12px', color: 'var(--text-secondary)', fontWeight: 600, textAlign: 'right' }}>Y Coord ({unitStr})</th>
              </tr>
            </thead>
            <tbody>
              {holeList.map((h) => (
                <tr key={h.id} style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.05)' }}>
                  <td style={{ padding: '8px 12px', color: 'var(--text-primary)', fontWeight: 600 }}>
                    <span style={{ display: 'inline-block', width: '22px', height: '22px', background: 'rgba(0, 255, 128, 0.15)', color: '#00ff80', textAlign: 'center', lineHeight: '22px', fontSize: '0.75rem', marginRight: '6px' }}>
                      {h.id}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: '8px 12px', textAlign: 'right', fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--accent-cyan)' }}>
                    {h.x >= 0 ? `+${h.x.toFixed(decPlaces)}` : h.x.toFixed(decPlaces)}
                  </td>
                  <td style={{ padding: '8px 12px', textAlign: 'right', fontFamily: 'var(--font-mono)', fontWeight: 700, color: '#38bdf8' }}>
